...
//     --include-python     include PeTTa Python FFI examples, running MeTTa-TS with --py
//     --include-prolog     include PeTTa prologimport, running MeTTa-TS with --prolog

import { spawnSync } from "node:child_process";
import { readdirSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
import { resolve, dirname, join, basename } from "node:path";
import { arg, benchDir as here, cliPath, flag, hashConsEnabled } from "./bench-common.mjs";
...
const INCLUDE_PYTHON = flag("include-python") || process.env.METTA_TS_INCLUDE_PYTHON === "1";
const INCLUDE_PROLOG = flag("include-prolog") || process.env.METTA_TS_INCLUDE_PROLOG === "1";
const QUICK_SET = [
  "fib",
  "fact",
  "ackermann",
...
const PYTHON_HOST_EXAMPLES = new Set(["python", "python_import", "torch"]);
const PROLOG_HOST_EXAMPLES = new Set(["prologimport"]);
...
const BASE_EXCLUSIONS = {
  ...
  callquoteevalreduce:
    "PeTTa call/eval/reduce full-evaluation vocabulary (Hyperon eval is single-step)",
  spaces_succeedspredicate: "PeTTa unify-as-space-query (unify a space against a pattern)",
  functionremoval:
    "PeTTa reverse function matching (predominantly; removes a function clause by inversion)",
  ...
  roman_test:
    "lib_roman uses PeTTa cons-list matching and = as a unification predicate (both PeTTa-only)",
  spaces_find:
    "needs PeTTa global-variable propagation: find's internal match must bind the caller's vars THROUGH collapse, which loses per-result bindings in Hyperon (variables are expression-scoped)",
  translatorrule:
    "PeTTa add-translator-rule! (compile-time macro/rewrite system), not a Hyperon feature",
  translatorrule_for: "PeTTa add-translator-rule! (compile-time macro/rewrite system)",
  test_unify_eval_branches:
    "PeTTa unify-as-space-query: lib_he overloads unify with an is-space dispatch so (unify &self pat then else) queries the space. Hyperon's unify is the minimal-MeTTa atom-vs-atom primitive (let = (unify $atom $pat $tmpl Empty)), so unifying the &self space atom against a pattern expression always fails and returns the else branch",
  patrick_test:
    "PeTTa reverse function matching (@) + add-translator-rule! (for) + cons-list matching",
  mettaset:
    "PeTTa superpose-as-union: (superpose ((1 (superpose (a b c))) ...)) yields 8 atoms in PeTTa. Hyperon/LeaTTa evaluate superpose's tuple argument as a cross-product first ({a,b,c}x{d,e,f}x{a,b} = 18 combos, each a 3-tuple superposed to 54 atoms), verified against the LeaTTa binary — the same cross-product semantics documented in spaces2",
  metta4_streams:
  ...
  ifsimple:
    "PeTTa 2-arg if (optional else): Hyperon's if is (-> Bool Atom Atom $t) (hyperon stdlib.metta:511), so (if True 42) is IncorrectNumberOfArguments. No Hyperon-faithful form of a 2-arg if",
  booleansolver:
    "PeTTa 2-arg if used as a relational solver: (if (and (or $x True) $y) ($x $y)) returns the then-branch under satisfiable bindings (Prolog-style), not a Hyperon construct",
  casenew:
  ...
  holfunctions_intrinsicop:
    "PeTTa cons-list matching: (= (mymap $f (cons $x $xs)) ...) over a flat tuple",
  logicprog:
    "PeTTa relational logic programming: (later-in-alphabet d $1) solved backward; the LeaTTa binary also does not terminate (verified)",
  logicprogset:
    "PeTTa 2-arg if + relational solving: (if (once (myf $M)) $M) with (member a $M) used to enumerate $M",
  scale:
    "PeTTa unify-as-space-query + let* over the space ((unify ($head $tail) $ht ...)); the LeaTTa binary also errors (bad-let-star) (verified)",
  matespace:
    "PeTTa Prolog-backed case/match saturation over a globally-persistent space. Hyperon-experimental and LeaTTa do not validate PeTTa's 1063919 constant for this file; the Hyperon-valid workload is matespacefast",
  matespace2:
    "PeTTa Prolog-backed snapshot/case saturation over a globally-persistent space. Hyperon-experimental and LeaTTa do not validate PeTTa's 1297533 constant for this file; the Hyperon-valid workload is matespacefast",
  specializefunctiontypes:
    "PeTTa specializer (f_Spec_[g], repra) — same class as the excluded specialize",
  translatepredicate:
    "PeTTa translatePredicate (the translator's compile-time rewrite) + Prolog-style (is $x 2)",
};
const EXCLUSIONS = { ...BASE_EXCLUSIONS };
if (INCLUDE_PYTHON) for (const name of PYTHON_HOST_EXAMPLES) delete EXCLUSIONS[name];
if (INCLUDE_PROLOG) for (const name of PROLOG_HOST_EXAMPLES) delete EXCLUSIONS[name];
...
function runTs(name) {
  const isPythonHost = PYTHON_HOST_EXAMPLES.has(name);
  const isPrologHost = PROLOG_HOST_EXAMPLES.has(name);
  return timeCmd(
    process.execPath,
    [
      "--stack-size=8000",
      ...
      ...(isPythonHost ? ["--py"] : []),
      ...(isPrologHost ? ["--prolog"] : []),
      join(isPythonHost || isPrologHost ? EXAMPLES : TS_CORPUS, name + ".metta"),
    ],
    { METTA_TS_STACK: "1" },
    isPrologHost ? PETTA_DIR : undefined,
  );
}
...
console.log(
  `  timeout=${TIMEOUT_MS / 1000}s runs=${RUNS} max-steps=${MAX_STEPS} engine=${ENGINE} hash-cons=${HASH_CONS} include-python=${INCLUDE_PYTHON} include-prolog=${INCLUDE_PROLOG}\n`,
);
...
console.log("\n" + "=".repeat(72));
console.log(
  `examples:            ${rows.length} (+${excluded.length} N/A, host capability outside pure TS)`,
);
...
if (both.length) {
  ...
  if (median)
    console.log(`speedup median:      ${median.toFixed(2)}x   geomean ${geomean.toFixed(2)}x`);
  console.log(
...
md.push(
  `- examples: ${rows.length}, both pass: ${bothPass.length}` +
    (median ? `, speedup median ${median.toFixed(2)}x, geomean ${geomean.toFixed(2)}x` : ""),
);
md.push(`- timeout ${TIMEOUT_MS / 1000}s, runs ${RUNS} (min), MeTTa-TS --max-steps ${MAX_STEPS}\n`);
if (INCLUDE_PYTHON || INCLUDE_PROLOG)
  md.push(
    `- host FFI included: Python ${INCLUDE_PYTHON ? "yes" : "no"}, Prolog ${INCLUDE_PROLOG ? "yes" : "no"}\n`,
  );
...
for (const r of rows) {
  ...
  md.push(
    `| ${r.name} | ${pm} | ${tm} | ${r.speedup ? r.speedup.toFixed(2) + "x" : "-"} | ${r.agree} |`,
  );
}
